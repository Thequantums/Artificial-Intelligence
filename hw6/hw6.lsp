...
;
(defun node2var (n c k) (+ (* (- n 1) k) c))

; EXERCISE: Fill this function
; returns *a clause* for the constraint:
; "node n gets at least (node2var n (- 0 c) k)one color from the set {c,c+1,...,k}."
;
(defun at-least-one-color (n c k)
  (cond ((equal c (+ k 1)) nil)
...
(defun permutate-negate-apply-color (n it min max)
  (cond ((= it max) nil)
        (t (cons (list (- 0 (node2var n min max)) (- 0 (node2var n (+ it 1) max))) (permutate-negate-apply-color n (+ it 1) min max)))))

(defun at-most-one-color (n c k)
  (cond ((equal c k) nil)
        (t (append (permutate-negate-apply-color n c c k) (at-most-one-color n (+ c 1) k)))))   
...
(defun generate-edge-helper (it e k)
  (cond ((equal it (+ k 1)) nil)
        (t (cons (list (- 0 (node2var (car e) it k)) (- 0 (node2var (cadr e) it k))) (generate-edge-helper (+ it 1) e k))))) 

(defun generate-edge-clauses (e k) (generate-edge-helper 1 e k))
